...
class FSPool(nn.Module):
    ...
    """
    def __init__(self, in_channels, n_pieces, relaxed=True):
        """
...
class DeepSetAutoencoder(torch.nn.Module):
        ...
        **kwargs
        ...
        for i in range(len(class_sizes)):
            cls_layers.append(nn.Sequential(
                nn.Linear(latent_dim*2 if i == 0 else class_sizes[i - 1], class_sizes[i]),
                nn.BatchNorm1d(class_sizes[i]) if use_bn and i != len(class_sizes) -1 else nn.Identity(),
                nn.ReLU())
            )

        cls_layers.append(nn.Linear(class_sizes[-1], 2))

        self.classifer = nn.Sequential(*cls_layers)
    ...
    def forward(self, features, mask):
        ...
        logits = self.classifer(torch.cat([mean, log_var], axis=1))

        return x, mean, log_var, z, logits
